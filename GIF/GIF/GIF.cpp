...
bool GIF::loadMesh()
{
	...
	if (mObjpath.size() == 0) {
		...
		double segSize = informationMatrix(0, 0), interiorTriCount = informationMatrix(1, 0), useElimination = informationMatrix(2, 0), performFinalGlobalScaling = informationMatrix(6, 0), operationMode = informationMatrix(7, 0);
		...
		mPerformFinalGlobalScaling = (performFinalGlobalScaling == 1);
		mPercentCurvatureMeta = informationMatrix(3, 0);
		mOuterRateTerminationCondition = informationMatrix(4, 0);
		mEnergyRelatedTerminationCondition = informationMatrix(5, 0);
	}
	...
	if (mOperationMode == 1)
	{
		mPercentCurvatureMeta = 0.001;
		mOuterRateTerminationCondition = 0.1;
		mEnergyRelatedTerminationCondition = 2;
